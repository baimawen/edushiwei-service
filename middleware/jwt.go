...
import (
	...
	"edushiwei-service/model/request"
	"edushiwei-service/model/response"
	"edushiwei-service/service"
	"errors"
	"strconv"
	"time"
...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if claims.ExpiresAt-time.Now().Unix() < claims.BufferTime {
			claims.ExpiresAt = time.Now().Unix() + global.COURSE_CONFIG.JWT.ExpiresTime
			...
			if global.COURSE_CONFIG.System.UseMultipoint {
				...
				if err != nil {
					global.COURSE_LOG.Error("get redis jwt failed", zap.Any("err", err))
...
func NewJWT() *JWT {
	return &JWT{
		[]byte(global.COURSE_CONFIG.JWT.SigningKey),
	}
}
